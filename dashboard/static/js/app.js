...
async function fetchOverviewSummary() {
    try {
        ...
        // Update permission requests pending badge
        const permBadge = document.getElementById('permissions-pending-badge');
        if (data.pending_permissions > 0) {
            permBadge.innerText = data.pending_permissions;
...
function switchPermTab(tab) {
    activePermTab = tab;
    ['pending', 'approved', 'rejected', 'impact'].forEach(t => {
        document.getElementById(`perm-tab-${t}`).classList.toggle('active', t === tab);
        document.getElementById(`perm-panel-${t}`).classList.toggle('hidden', t !== tab);
    });

    // Render impact when selected
    if (tab === 'impact') renderPermImpact();
}
...
async function loadPermissions() {
    try {
        ...
        document.getElementById('perm-pending-list').innerHTML =
            '<div class="empty-state"><i class="fa-solid fa-triangle-exclamation"></i><p>Failed to load permission requests.</p></div>';
    }
}
...
function renderPermissions(data) {
    ...
    // Update tab counts
    document.getElementById('perm-count-pending').textContent = pending.length;
    document.getElementById('perm-count-approved').textContent = approved.length;
    document.getElementById('perm-count-rejected').textContent = rejected.length;

    // Update sidebar badge
    const permBadge = document.getElementById('permissions-pending-badge');
    if (pending.length > 0) {
        permBadge.innerText = pending.length;
        permBadge.classList.remove('hidden');
    } else {
        permBadge.classList.add('hidden');
    }
    ...
    const pendingList = document.getElementById('perm-pending-list');
    if (pending.length === 0) {
        pendingList.innerHTML = `
            <div class="empty-state" style="grid-column:1/-1;">
                <i class="fa-solid fa-check-double" style="font-size:48px;color:var(--success);margin-bottom:16px;"></i>
                <p>No pending permission requests. All caught up! ✅</p>
            </div>`;
    } else {
        pendingList.innerHTML = pending.map(r => buildPermCard(r)).join('');
    }

    // Render approved table
    const approvedBody = document.getElementById('perm-approved-list');
    approvedBody.innerHTML = approved.length === 0
        ? '<tr><td colspan="9" style="text-align:center;color:var(--text-secondary);">No approved requests found.</td></tr>'
        : approved.map(r => buildPermHistoryRow(r, 'approved')).join('');

    // Render rejected table
    const rejectedBody = document.getElementById('perm-rejected-list');
    rejectedBody.innerHTML = rejected.length === 0
        ? '<tr><td colspan="8" style="text-align:center;color:var(--text-secondary);">No rejected requests found.</td></tr>'
        : rejected.map(r => buildPermHistoryRow(r, 'rejected')).join('');
}

function buildPermCard(r) {
    const typeLabel = PERM_TYPE_LABELS[r.request_type] || r.request_type;
    const typeClass = `perm-type-${r.request_type}`;
    const durationStr = secondsToHMS(r.duration_seconds);

    return `
    <div class="perm-card" id="perm-card-${r.id}">
        <div class="perm-card-header">
            <div>
                <div class="perm-card-employee">${escapeHtml(r.name)}</div>
                <div class="perm-card-username">${escapeHtml(r.username && r.username !== 'NoUsername' ? '@' + r.username : '')}</div>
            </div>
            <span class="perm-type-badge ${typeClass}">${typeLabel}</span>
        </div>
        <div class="perm-card-body">
            <div class="perm-card-field">
                <span class="perm-card-label">Date</span>
                <span class="perm-card-value">${r.date}</span>
            </div>
            <div class="perm-card-field">
                <span class="perm-card-label">Duration</span>
                <span class="perm-card-value">${durationStr}</span>
            </div>
            <div class="perm-card-field">
                <span class="perm-card-label">Start Time</span>
                <span class="perm-card-value">${r.start_time.slice(0, 5)}</span>
            </div>
            <div class="perm-card-field">
                <span class="perm-card-label">End Time</span>
                <span class="perm-card-value">${r.end_time.slice(0, 5)}</span>
            </div>
            <div class="perm-card-reason">
                <i class="fa-solid fa-quote-left" style="opacity:0.4;margin-right:6px;"></i>${escapeHtml(r.reason)}
            </div>
        </div>
        <div class="perm-card-actions">
            <button class="btn-approve" onclick="handlePermDecision(${r.id}, 'approve')">
                <i class="fa-solid fa-circle-check"></i> Approve
            </button>
            <button class="btn-reject" onclick="handlePermDecision(${r.id}, 'reject')">
...
function buildPermHistoryRow(r, status) {
    const typeLabel = PERM_TYPE_LABELS[r.request_type] || r.request_type;
    const decidedAt = r.decided_at ? r.decided_at.slice(0, 16) : '—';
    const approverName = r.approver_name || '—';
    const durationStr = secondsToHMS(r.duration_seconds);

    let notifyStatusHtml = '';
    if (r.notification_status === 'sent') {
        notifyStatusHtml = `<span style="color:var(--success);font-weight:600;"><i class="fa-solid fa-circle-check"></i> Sent</span>`;
    } else if (r.notification_status === 'failed') {
        notifyStatusHtml = `
            <div style="display:flex;align-items:center;gap:6px;">
                <span style="color:var(--danger);font-weight:600;"><i class="fa-solid fa-circle-xmark"></i> Failed</span>
                <button class="btn-retry-notify" onclick="retryNotification(${r.id}, this)" title="Retry Notification" style="padding:2px 6px;font-size:10px;border-radius:4px;border:1px solid var(--danger);background:transparent;color:var(--danger);cursor:pointer;transition:all 0.2s;">
        ...
        notifyStatusHtml = `
            <div style="display:flex;align-items:center;gap:6px;">
                ...
                <button class="btn-retry-notify" onclick="retryNotification(${r.id}, this)" title="Send Notification" style="padding:2px 6px;font-size:10px;border-radius:4px;border:1px solid var(--text-secondary);background:transparent;color:var(--text-secondary);cursor:pointer;transition:all 0.2s;">
    ...
    if (status === 'approved') {
        return `<tr>
            <td>${r.id}</td>
            <td><strong>${escapeHtml(r.name)}</strong><br><small>@${escapeHtml(r.username || '')}</small></td>
            <td>${typeLabel}</td>
            <td>${r.date}</td>
            <td>${r.start_time.slice(0, 5)} → ${r.end_time.slice(0, 5)}</td>
            <td>${durationStr}</td>
            <td style="max-width:180px;word-break:break-word;">${escapeHtml(r.reason)}</td>
            <td>${escapeHtml(approverName)}</td>
            <td>${decidedAt}</td>
            <td>${notifyStatusHtml}</td>
        </tr>`;
    } else {
        return `<tr>
            <td>${r.id}</td>
...
function renderPermImpact() {
    const container = document.getElementById('perm-impact-content');
    const data = permissionsCache;

    const totalRequests = data.length;
    const totalApproved = data.filter(r => r.status === 'approved').length;
    const totalRejected = data.filter(r => r.status === 'rejected').length;
    const totalPending = data.filter(r => r.status === 'pending').length;
    const totalCredSec = data
        .filter(r => r.status === 'approved')
        .reduce((s, r) => s + (r.duration_seconds || 0), 0);
    const uniqueEmployees = new Set(data.map(r => r.telegram_id)).size;

    const approvalRate = totalRequests > 0 ? Math.round((totalApproved / totalRequests) * 100) : 0;

    container.innerHTML = `
        <div class="perm-impact-card">
            <div class="impact-value">${totalRequests}</div>
            <div class="impact-label">Total Requests Submitted</div>
        </div>
        <div class="perm-impact-card">
            <div class="impact-value" style="background:linear-gradient(135deg,#10b981,#059669);-webkit-background-clip:text;background-clip:text;">${totalApproved}</div>
            <div class="impact-label">Requests Approved</div>
        </div>
        <div class="perm-impact-card">
            <div class="impact-value" style="background:linear-gradient(135deg,#ef4444,#dc2626);-webkit-background-clip:text;background-clip:text;">${totalRejected}</div>
            <div class="impact-label">Requests Rejected</div>
        </div>
        <div class="perm-impact-card">
            <div class="impact-value" style="background:linear-gradient(135deg,#f59e0b,#d97706);-webkit-background-clip:text;background-clip:text;">${totalPending}</div>
            <div class="impact-label">Pending Review</div>
        </div>
        <div class="perm-impact-card">
            <div class="impact-value">${secondsToHMS(totalCredSec)}</div>
            <div class="impact-label">Total Hours Credited</div>
        </div>
        <div class="perm-impact-card">
            <div class="impact-value">${uniqueEmployees}</div>
            <div class="impact-label">Unique Employees</div>
        </div>
        <div class="perm-impact-card">
            <div class="impact-value">${approvalRate}%</div>
            <div class="impact-label">Approval Rate</div>
        </div>
    `;
}

async function handlePermDecision(requestId, action) {
    const endpoint = `/api/permissions/${requestId}/${action}`;
    ...
    try {
        const res = await fetch(endpoint, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ approver_name: 'Admin (Dashboard)' }),
        });
        const result = await res.json();

        if (result.success) {
            const verb = action === 'approve' ? 'approved' : 'rejected';
            pushToast(`✅ Request #${requestId} ${verb}. Employee notified.`, 'success');
            // Reload the permission list
            await loadPermissions();
            // Refresh the sidebar badge counts
            await fetchOverviewSummary();
        } else {
            pushToast(`⚠️ ${result.error || 'Action failed.'}`, 'error');
...
async function retryNotification(requestId, btn) {
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = `<i class="fa-solid fa-spinner fa-spin"></i> Retrying...`;
    }
    try {
        const res = await fetch(`/api/permissions/${requestId}/retry-notification`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' }
        });
        const result = await res.json();
        if (result.success) {
            pushToast('✅ Telegram notification sent successfully!', 'success');
            await loadPermissions();
        } else {
            pushToast('❌ Failed to send Telegram notification. Please try again.', 'error');
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = `<i class="fa-solid fa-arrow-rotate-right"></i> Retry`;
            }
        }
    } catch (err) {
        console.error('Error retrying notification:', err);
        pushToast('⚠️ Network error. Please try again.', 'error');
        if (btn) {
            btn.disabled = false;
            btn.innerHTML = `<i class="fa-solid fa-arrow-rotate-right"></i> Retry`;
        }
